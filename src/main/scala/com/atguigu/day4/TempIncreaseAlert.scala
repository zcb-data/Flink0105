...
/**
 * @author ：ZCB
 * @date ：Created in 2021/8/23 21:14
 * @desc：
 */
object TempIncreaseAlert {
  ...
  class TempIncreaseAlertFunction extends KeyedProcessFunction[String,SensorReading,String]{
    ...
    //懒加载，惰性赋值
    //当执行到process算子时，才会初始化，所示是懒加载。
    //为什么不直接使用scala的变量呢，比如：var lastTemp:Double = _
    //通过配置，状态变量可以通过检查的操作，保存在hdfs里面
    //当程序故障时，可以从最近一次检查点恢复
    //所以要有一个名字”last-temp“的变量的类型
     //状态变量只会初始化一次，运行程序时，如果没有这个状态变量，就初始化一个
    //如果有这个状态变量，直接读取
    //所以是单例模式
    // 默认值是0.0
    ...
    val prevTemp: Double = lastTemp.value()
      ...
      if (prevTemp==0.0 || value.temperature < prevTemp){
        ...
        //来的温度大于最近一次温度，并且没有注册报警定时器，因为curTimerTs == 0L
